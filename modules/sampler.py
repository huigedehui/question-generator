...
class QuestionSampler:
    ...
    def sample_questions(self, questions: List[Dict], 
                        counts: Dict[str, int],
                        strategy: str = "quality") -> Dict[str, List[Dict]]:
        """
        ...
        Args:
            ...
            strategy: Sampling strategy ("random", "quality", "first", "weighted")
        ...
        for q_type, count in counts.items():
            ...
            if len(filtered) < count:
                ...
                if strategy == "quality":
                    filtered.sort(key=lambda q: q.get("_quality", 10), reverse=True)
...
def sample_questions(cleaned_questions: List[Dict], 
                     ...
                     strategy: str = "quality") -> Dict[str, List[Dict]]:
    """
    Convenience function for quality-weighted sampling.
    
    Args:
        cleaned_questions: List of cleaned questions
        counts: Dict of {type: count}
        strategy: Sampling strategy ("quality", "random", "first")
    ...
    result = sampler.sample_questions(cleaned_questions, counts, strategy=strategy)
