...
class QuestionValidator:
    ...
    def quality_score(self, question: Dict) -> float:
        q_type = question.get("type", "")
        options = question.get("options", [])
        ...
        if q_type in ("单选", "多选") and len(options) >= 2:
            lengths = [len(o) for o in options]
            avg = sum(lengths) / len(lengths)
            if max(lengths) > avg * 1.8: score -= 2
            if min(lengths) < avg * 0.3: score -= 1.5

            correct_indicators = set(answer.replace(" ", "").split(","))
            wrong_opts = [o for i, o in enumerate(options) if chr(65+i) not in correct_indicators]
            for w in wrong_opts:
                if len(w) < 5: score -= 1

            bad = {"以上都对", "以上都错", "以上都不是", "以上皆", "全部都对", "全部错误"}
            for b in bad:
                if any(b in o for o in options): score -= 3

        if len(content) < 15: score -= 2
        if len(analysis) < 10: score -= 1.5
